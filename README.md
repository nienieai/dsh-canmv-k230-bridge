...
- **脚本载入**：📂 打开脚本（资源管理器）或拖放 `.py` 到面板 → ▶ 运行脚本（自动开启预览）
- **实时预览**：👁 FB 帧缓冲 JPEG；「面板状态」小框与「开发板调试」框分流显示
- **设置页「CanMV」**：可开关悬浮窗
- **11 个模型工具**：端口 / 执行 / 上传 / 文件系统 / 复位 / 信息 / 运行文件 / 取帧 / 预览 / 会话 / 停止
- **不限时会话**：后台进程运行，无 10 分钟钳制；内置帧冻结哨兵（60s 无帧自动 FB 重武装并标记）
...
然后 `cordis_run` 激活；Client 半侧首次激活需页面审批，建议勾选「信任未来版本」。
...
- 0x30 + cmd + uint32LE 帧：0x8D 同步 / 0x11 软复位 / 0x05 SCRIPT_EXEC /
